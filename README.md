...
연결 복구와 전송 처리는 다음과 같이 구성되어 있습니다.

- 포인터와 통화 데이터 채널은 열리기 전이나 전송 버퍼가 찼을 때 메시지를 대기시킵니다. 참가자 상태는 최신 값으로 합치고, 음성 연결 신호는 순서를 유지합니다.
- 영상 연결과 참가자별 음성 연결은 응답 대기 시간과 재시도 횟수를 제한합니다. 이전 연결의 신호는 식별자로 구분하며, 재협상과 송신 트랙 변경은 연결별·송신자별로 순서대로 처리합니다.
- 일시적인 WebSocket 재접속에는 복구 유예 시간을 두어 기존 P2P 연결을 유지하고, 연결을 새로 만드는 요청은 별도로 처리합니다.
- 원본 해상도와 최대 60fps 설정은 연결 및 재연결 후 다시 적용합니다. 음성 재생이 브라우저에 의해 중단되면 화면 복귀나 사용자 조작 시 재개를 시도합니다.
